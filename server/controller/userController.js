  ...
  try{
    ...
    if(req.user.role === "admin"){
      ...
      if(user.role === "employee"){
        ...
        res.status(403).json({ message: "Forbidden: Managers can only update employees" });
      }
    }
      ...
      if(user.id === req.user.id){
        ...
        res.status(403).json({ message: "Forbidden: Employees can only update their own profile" });
      }
    }
