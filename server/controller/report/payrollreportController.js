...
const buildPayrollUserInclude = (department_id) => ({
  model: User,
  as: "user",
  ...
  try {
    ...
    const rows = await Payroll.findAll({
      ...
      attributes: [
        ...
        [fn("SUM",   literal(`CASE WHEN "Payroll"."status" = 'paid'     THEN "Payroll"."net_salary" ELSE 0 END`)), "total_paid"],
        [fn("SUM",   literal(`CASE WHEN "Payroll"."status" = 'pending'  THEN "Payroll"."net_salary" ELSE 0 END`)), "total_pending"],
        [fn("SUM",   literal(`CASE WHEN "Payroll"."status" = 'approved' THEN "Payroll"."net_salary" ELSE 0 END`)), "total_approved"],
      ],
      include: [userInclude],
...
export const getPayrollTrends = async (req, res) => {
  try {
    ...
    const includeClause = needsJoin
      ? [applyManagerScope(req, buildPayrollUserInclude(department_id))]
      : [];
    ...
    const rows = await Payroll.findAll({
      ...
      attributes: [
        ...
        "month",
        [fn("COUNT", col("Payroll.id")),                                      "payroll_count"],
        [fn("COUNT", literal(`DISTINCT "Payroll"."user_id"`)),                "employee_count"],
        [fn("SUM",   col("Payroll.base_salary")),                             "total_base_salary"],
        [fn("SUM",   col("Payroll.bonus")),                                   "total_bonus"],
        [fn("SUM",   col("Payroll.allowance")),                               "total_allowance"],
        [fn("SUM",   col("Payroll.deductions")),                              "total_deductions"],
    ...
    return res.status(500).json({ success: false, message: "Failed to fetch payroll trends" });
  }
};
...
export const getSalaryAnalytics = async (req, res) => {
  try {
    ...
    const [overallStats, topEarners, byDepartment, byStatus] = await Promise.all([
      ...
      Department.findAll({
        attributes: [
          "id", "name", "code",
          [fn("COALESCE", fn("SUM",   col("employees->payrolls.net_salary")),  literal("0")), "total_net_salary"],
          [fn("COALESCE", fn("SUM",   col("employees->payrolls.base_salary")), literal("0")), "total_base_salary"],
          [fn("COALESCE", fn("SUM",   col("employees->payrolls.bonus")),       literal("0")), "total_bonus"],
          [fn("COALESCE", fn("SUM",   col("employees->payrolls.deductions")),  literal("0")), "total_deductions"],
          [fn("COALESCE", fn("AVG",   col("employees->payrolls.net_salary")),  literal("0")), "avg_net_salary"],
          [fn("COUNT",    col("employees->payrolls.id")),                                      "payroll_count"],
          [fn("COUNT",    literal(`DISTINCT "employees"."id"`)),                               "employee_count"],
        ],
        include: [
          {
            model: User,
        ...
        group:   ["Department.id"],
        order:   [[fn("COALESCE", fn("SUM", col("employees->payrolls.net_salary")), literal("0")), "DESC"]],
        subQuery: false,
      }),
    ...
    return res.status(500).json({ success: false, message: "Failed to fetch salary analytics" });
  }
};