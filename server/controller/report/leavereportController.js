...
const EXCLUDED_FIELDS = [
  "password", "resetPasswordToken", "resetPasswordExpires", "refreshTokenHash",
];
...
const buildLeaveUserInclude = (department_id) => ({
  model: User,
  as: "user",
  attributes: ["id", "name", "email", "employee_id", "position"],
...
export const getLeaveSummary = async (req, res) => {
  try {
    ...
    const userInclude = applyManagerScope(
      req,
      buildLeaveUserInclude(department_id),
    );
    ...
    const rows = await Leave.findAll({
      ...
      attributes: [
        ...
        [fn("SUM", literal(`CASE WHEN "Leave"."overall_status" = 'approved' THEN 1 ELSE 0 END`)),               "approved"],
        [fn("SUM", literal(`CASE WHEN "Leave"."overall_status" IN ('pending_manager','pending_hr') THEN 1 ELSE 0 END`)), "pending"],
        [fn("SUM", literal(`CASE WHEN "Leave"."overall_status" IN ('rejected_by_manager','rejected_by_hr') THEN 1 ELSE 0 END`)), "rejected"],
        [fn("SUM", literal(`CASE WHEN "Leave"."overall_status" = 'approved' THEN "Leave"."days" ELSE 0 END`)),  "approved_days"],
    ...
    return res.status(500).json({ success: false, message: "Failed to generate leave summary" });
  }
};
...
export const getLeaveDetails = async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      pagination: {
        total: count, page: pageNum, limit: limitNum, totalPages,
...
export const getLeaveTrends = async (req, res) => {
  try {
    ...
    const includeClause = needsJoin
      ? [applyManagerScope(req, buildLeaveUserInclude(department_id))]
      : [];
    ...
    const rows = await Leave.findAll({
      ...
      attributes: [
        ...
        [fn("SUM", literal(`CASE WHEN "Leave"."overall_status" IN ('rejected_by_manager','rejected_by_hr') THEN 1 ELSE 0 END`)), "rejected"],
        [fn("SUM", literal(`CASE WHEN "Leave"."overall_status" = 'approved' THEN "Leave"."days" ELSE 0 END`)), "approved_days"],
        [fn("SUM", literal(`CASE WHEN "Leave"."type" = 'annual'    THEN 1 ELSE 0 END`)),                       "annual"],
        [fn("SUM", literal(`CASE WHEN "Leave"."type" = 'sick'      THEN 1 ELSE 0 END`)),                       "sick"],
        [fn("SUM", literal(`CASE WHEN "Leave"."type" = 'casual'    THEN 1 ELSE 0 END`)),                       "casual"],
        [fn("SUM", literal(`CASE WHEN "Leave"."type" = 'emergency' THEN 1 ELSE 0 END`)),                       "emergency"],
        [fn("SUM", literal(`CASE WHEN "Leave"."type" = 'unpaid'    THEN 1 ELSE 0 END`)),                       "unpaid"],
      ],
    ...
    return res.status(500).json({ success: false, message: "Failed to fetch leave trends" });
  }
};
...
export const getLeaveBalance = async (req, res) => {
  try {
    ...
    const { count: totalUsers, rows: users } = await User.findAndCountAll({
      ...
      include: [{ model: Department, as: "dept", attributes: ["id", "name", "code"], required: false }],
      order: [["name", "ASC"]],
      limit: limitNum,
      offset,
      distinct: true,
    ...
    if (users.length === 0) {
      return res.json({
        success: true,
        pagination: { total: totalUsers, page: pageNum, limit: limitNum,
          totalPages: Math.ceil(totalUsers / limitNum), hasNext: false, hasPrev: pageNum > 1 },
        data: [],
      });
    }
    ...
    const data = users.map((user) => {
      ...
      return {
        user: { id: user.id, name: user.name, email: user.email,
                employee_id: user.employee_id, position: user.position, department: user.dept ?? null },
        balance: {
          annual:    { allocated: annual,  used: used.annual    || 0, remaining: annual  - (used.annual    || 0) },
          sick:      { allocated: sick,    used: used.sick      || 0, remaining: sick    - (used.sick      || 0) },
          casual:    { allocated: casual,  used: used.casual    || 0, remaining: casual  - (used.casual    || 0) },
    ...
    return res.json({
      success: true,
      pagination: { total: totalUsers, page: pageNum, limit: limitNum, totalPages,
        hasNext: pageNum < totalPages, hasPrev: pageNum > 1 },
      data,
    });
  } catch (error) {
